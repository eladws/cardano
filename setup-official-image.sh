...
echo "* 6 * * * sudo docker-compose -f cardano/docker-compose-official-image.yaml restart relay-node" >> crontab.txt
crontab crontab.txt
...
# start node
sudo docker-compose -f cardano/docker-compose-official-image.yaml up -d
